          ...
          <div
            className={`relative border border-solid ${
              valUser.subscriptionPlan?.data?.name == "Black"
                ? "border-white"
                : "border-black"
            } overflow-hidden rounded-3xl flex flex-row w-full justify-between xl:gap-12 2xl:gap-16 cursor-default`}
            // style={{ backgroundColor: valUser ? valUser.subscriptionPlan?.data?.color : "" }}
            style={{
              background: `linear-gradient(90deg, ${
                valUser._id ? valUser.subscriptionPlan?.data?.color : "white"
              } 0%, ${
                valUser._id
                ...
                <span className="text-xs font-bold xl:text-xl 2xl:text-2xl special:text-3xl">
                  ...
                  >
                    ...
                    {valUser.subscription_status === "unsubscribed" ? (
                      <>
                        <span className="font-bold xl:text-xs hidden">
                          Current package unsubscribed on&nbsp;{unsubDate},
                        </span>
                        &nbsp;Subscription ends on&nbsp;
                        <span className="xl:text-xs">{endDate}</span>
                      </>
                    ) : valUser.subscription_status === "expired" ? (
                      <>
                        <span className="xl:text-xs">Expire Date On</span>&nbsp;
                        <span className="xl:text-xs">{endDate}</span>
                      </>
                    ) : valUser.subscription_status === "active" ? (
                      <>
                        <span className="xl:text-xs">Auto Renews On</span>&nbsp;
                        {endDate}
                      </>
                    ) : valUser.subscription_status === "noplan" ? (
